...
def start():
    ...
    typer.secho("[RAG] Welcome to Antigravity Agentic RAG CLI!", fg=typer.colors.CYAN, bold=True)
    
    # Pre-flight check: Verify Typhoon API Key
    api_key = os.getenv("TYPHOON_API_KEY")
    if not api_key or api_key == "your_typhoon_api_key_here" or api_key == "mock_key_for_testing":
    ...
    try:
        ...
        typer.secho(f"\n[Warning] Setup Alert: {val_err}", fg=typer.colors.YELLOW)
        raise typer.Exit(code=1)
    except Exception as e:
        typer.secho(f"\n[Error] Initialization Error: {e}", fg=typer.colors.RED, err=True)
        raise typer.Exit(code=1)

    typer.secho("\n[Ready] RAG Agent is ready! (Type 'exit' or 'quit' to close the session)\n", fg=typer.colors.GREEN, bold=True)
    ...
    while True:
        ...
        typer.secho("[RAG] Thinking and routing through document collections...", fg=typer.colors.BLUE)
