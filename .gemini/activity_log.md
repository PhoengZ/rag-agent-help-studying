...
## [2026-06-03] - Typhoon OCR Rate Limiting Cooldown
- **Attempted**: Implemented a thread-safe rate-limiting cooldown inside `TyphoonOCRReader._ocr_page` utilizing a `threading.Lock` and keeping track of the elapsed time between consecutive API calls.
- **Hypothesis**: Restricting the rate of API calls to at least 3.0 seconds between request initiations (based on the 20 requests/minute rate limit) will prevent HTTP 429 Rate Limit errors without sacrificing multi-threaded parallel page downloads once the requests are launched.
- **Observed Result/Outcome**: Created and ran a multi-threaded mock test script `scratch/test_typhoon_rate_limit.py`. The output verified that consecutive request start times were spaced exactly 3.0 seconds apart, completing 4 page requests in exactly 9.01 seconds. Verified no rate-limit breaches occurred.
